...
#Stemming is a process that removes suffixes from words, which can help in reducing word variants to a common base form.
    def _stem_words(self, words):
        ps = PorterStemmer() # This is a stemming algorithm provided by the nltk.stem module.
        stemmed_words = [ps.stem(word) for word in words] #Applies the stem method to each word in the list, producing a list of stemmed words.
        return stemmed_words

#lemmatization considers the context and converts words to their meaningful base forms.
    def _lemmatize_words(self, words):
        lemmatizer = WordNetLemmatizer() #This is a lemmatizer provided by the nltk.stem module that uses the WordNet lexical database.
        lemmatized_words = [lemmatizer.lemmatize(word) for word in words] #Applies the lemmatize method to each word in the list, producing a list of lemmatized words.
...
if __name__ == "__main__":
    ...
    analyzed.visualize_word_frequency(top_n=5)